    ...
    vec![
        ...
        ToolDescriptor::with_args(
            ...
            vec![
                ...
                string("mode").described("Read mode is cache-first by default. If refresh=true and refresh_mode is omitted, non-instant modes are used for the background scan."),
        ...
        ToolDescriptor::with_args(
            ...
            "Return a compact cache-first storage overview: summary, top findings, guardrails, top repo footprints, cache status, and diagnostics. Never waits on a deep scan.",
            vec![
                string_array("roots", Some(24)).described("Optional absolute paths or ~/ paths."),
                uint("max_depth", Some(1), Some(12), Some(5)),
                uint("limit", Some(1), Some(200), Some(80)),
                string("mode").described("Read mode is cache-first by default; non-instant values only affect the optional background refresh."),
                boolean("refresh", Some(false)).described("Return cached facts immediately and start a background scan."),
                boolean("background_scan", Some(false)).described("Alias for refresh; useful for clients that name side effects explicitly."),
                string("refresh_mode").described("Background scan mode: fast_changed_only, deep_native, or forensic_verified."),
                string("throttle_hint").described("Optional background throttle hint such as normal, battery, thermal-pressure, network, or cloud."),
                string_array("dirty_paths", Some(64)).described("Optional paths to prioritize during the background refresh."),
            ],
        ...
        ToolDescriptor::with_args(
            ...
            "Return cache-first cleanup tiers, recipes, bundles, guardrails, and diagnostics without the raw artifact list. Never waits on a deep scan.",
            vec![
                string_array("roots", Some(24)).described("Optional absolute paths or ~/ paths."),
                uint("max_depth", Some(1), Some(12), Some(5)),
                uint("limit", Some(1), Some(200), Some(80)),
        ...
        ToolDescriptor::with_args(
            ...
            "Return one page of ranked storage items plus cache status and diagnostics. Default pages are served directly from the persistent index and never wait on a deep scan.",
            vec![
                string_array("roots", Some(24)).described("Optional absolute paths or ~/ paths."),
                uint("max_depth", Some(1), Some(12), Some(5)),
        ...
        ToolDescriptor::with_args(
            ...
            "Return cache-first storage detail for one repository root: repo intelligence, top artifacts, cleanup actions, cache status, and diagnostics.",
            vec![
        ...
        ToolDescriptor::with_args(
            ...
            "Start an explicit background deep storage scan using deep_native mode and return job status immediately. Does not wait for scan results.",
            vec![
                string_array("roots", Some(24)).described("Optional absolute paths or ~/ paths."),
