...
# staple) -> generate/append the Sparkle appcast -> third-party notices. Reads
# the same AETOWER_* environment as the individual scripts (see
...
printf '\n=== 1/4 release preflight ===\n'
sh "$ROOT/scripts/release-preflight.sh"

printf '\n=== 2/4 package (build + sign + notarize) ===\n'
sh "$ROOT/scripts/package-macos.sh"

printf '\n=== 3/4 generate Sparkle appcast ===\n'
sh "$ROOT/scripts/generate-sparkle-appcast.sh"

printf '\n=== 4/4 generate third-party notices ===\n'
